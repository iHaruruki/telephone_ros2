// A 版とほぼ同じ。publisher のトピックだけ "audio_b_out" に変更。
#include <chrono>
#include <memory>
#include <vector>
#include <iostream>
...
class MicPublisherBNode : public rclcpp::Node
{
public:
  MicPublisherBNode()
  : Node("mic_publisher_b_node")
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "[B] Mic publisher: rate=%d Hz, channels=%d, chunk=%d ms (%d samples)",
    ...
    RCLCPP_INFO(get_logger(), "[B] Using input device: %s", info->name);
    ...
    RCLCPP_INFO(get_logger(), "[B] Mic publisher started.");
  }

  ~MicPublisherBNode() override
  {
    if (stream_) {
    ...
    Pa_Terminate();
    RCLCPP_INFO(get_logger(), "[B] Mic publisher stopped.");
  }
    ...
    void *userData)
  {
    ...
    (void)timeInfo;
    (void)statusFlags;
    auto *node = static_cast<MicPublisherBNode*>(userData);
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<MicPublisherBNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}